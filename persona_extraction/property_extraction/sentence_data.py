...
def calculate_num_sentences(paragraph):
    # Use regex to split the paragraph into sentences
    sentences = re.split(r'[.!?]', paragraph)
    # Remove empty strings from the list (due to trailing punctuation)
    sentences = [sentence.strip() for sentence in sentences if sentence.strip()]
    # Count the number of sentences
    num_sentences = len(sentences)
    return num_sentences

def calculate_sentence_length_frequency(paragraph):
    # Use regex to split the paragraph into sentences
    sentences = re.split(r'[.!?]', paragraph)
    # Remove empty strings from the list (due to trailing punctuation)
    sentences = [sentence.strip() for sentence in sentences if sentence.strip()]
    ...
    # Count the frequency of sentence lengths
    length_frequency = Counter(sentence_lengths)
    return dict(length_frequency)
...
def main():
    ...
    # Calculate the frequency of sentence lengths
    length_frequency = calculate_sentence_length_frequency(paragraph)

    # Prepare the output as JSON
    output = {
        "num_sentences": num_sentences,
        "sentence_length_frequency": length_frequency
    }
