from persona_extraction.property_extraction.word_frequency import calculate_word_frequency
from persona_extraction.property_extraction.sentence_data import calculate_sentence_length_frequency, calculate_num_sentences
from persona_extraction.property_extraction.punctuation import calculate_punctuation_frequency
from persona_extraction.property_extraction.number_of_paragraphs import count_paragraphs
from persona_extraction.property_extraction.mispelled import count_misspelled_words
from persona_extraction.data_generator import person_data
...
class Persona:
    ...
    def get_persona_json(self, paragraph):

   
        persona = {
            "num_paragraphs": count_paragraphs(paragraph),
            "num_sentences": calculate_num_sentences(paragraph),
            # "sentence_length_frequency": calculate_sentence_length_frequency(paragraph),
            "word_frequency" : calculate_word_frequency(paragraph=paragraph),
            # "mispelled_words_frequency" : count_misspelled_words(paragraph),
...
for topic in person_data:
    person_persona[topic] = persona_classifier.get_persona_json(person_data[topic])

# from pprint import pprint
print(person_persona)